# Auto-extracted and adapted script for local execution
...
PARAM$hipeparametertuning$hs <- makeParamSet(
  ...
  # --- Optimización de regularizadores sugerida por el profesor ---
...
# --- CELL 77 ---
# 1. Unimos los parámetros fijos originales
fijos <- copy(PARAM$lgbm$param_fijos)

# 2. Quitamos los parámetros que optimizamos en la Bayesiana para no duplicar
fijos$num_iterations <- NULL
fijos$early_stopping_rounds <- NULL

# 3. Creamos el set final aplicando el desempaque exponencial (2^x)
...
# --- Desempaquetamos la regularización optimizada ---
param_final$lambda_l1 <- x$lambda_l1
param_final$lambda_l2 <- x$lambda_l2
param_final$min_gain_to_split <- x$min_gain_to_split

# 4. Imprimimos el resultado final en consola para verificarlo
param_final
