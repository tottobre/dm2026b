...
r_script_path = "/Users/tomas/Dev/dm2026b/src/workflows/run_scoring_standalone.R"
with open(r_script_path, "w") as f:
    f.write(r_scoring_script)

print("Executing standalone scoring pipeline...")
res = subprocess.run(["Rscript", r_script_path], capture_output=True, text=True)
print("STDOUT:\n", res.stdout)
print("STDERR:\n", res.stderr)

if res.returncode == 0:
    print("\nSUCCESS! Submitting predictions to Kaggle...")
    kaggle_dir = "/Users/tomas/Dev/dm2026b/exp/WFjunior_rank_cero_fijo_semilla_primigenia_BO_ensemble/kaggle"
    target_cuts = ["submission_11000.csv", "submission_10500.csv", "submission_11500.csv"]
    
    for f in target_cuts:
        full_path = os.path.join(kaggle_dir, f)
        if os.path.exists(full_path):
            print(f"Submitting {f} to Kaggle competition 'data-mining-junior-2026-b'...")
            cmd = ["kaggle", "competitions", "submit", "-c", "data-mining-junior-2026-b", "-f", full_path, "-m", f"Junior BO + 5-seed Ensemble ({f})"]
            sub_res = subprocess.run(cmd, capture_output=True, text=True)
            print(f"Kaggle Result for {f}:\nSTDOUT: {sub_res.stdout}\nSTDERR: {sub_res.stderr}\n")
