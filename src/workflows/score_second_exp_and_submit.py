...
r_script_path = "/Users/tomas/Dev/dm2026b/src/workflows/run_second_exp_scoring.R"
with open(r_script_path, "w") as f:
    f.write(r_scoring_script)

print("Executing 20-seed scoring pipeline...")
res = subprocess.run(["Rscript", r_script_path], capture_output=True, text=True)
print("STDOUT:\n", res.stdout)
print("STDERR:\n", res.stderr)

if res.returncode == 0:
    print("\nSUCCESS! Submitting 20-seed predictions to Kaggle...")
    kaggle_dir = "/Users/tomas/Dev/dm2026b/exp/todas_sugerencias_BO_ensemble_y_sugerencia_antigrav/kaggle"
    ...
    for f in target_cuts:
        ...
        if os.path.exists(full_path):
            ...
            cmd = [
                ...
                "-c", "data-mining-junior-2026-b",
                "-f", full_path,
                "-m", f"Second Exp 20-Seed Ensemble + Antigravity FE & ExtraTrees ({f})"
            ]
            sub_res = subprocess.run(cmd, capture_output=True, text=True)
            print(f"Kaggle Submission Result for {f}:\nSTDOUT: {sub_res.stdout}\nSTDERR: {sub_res.stderr}\n")
