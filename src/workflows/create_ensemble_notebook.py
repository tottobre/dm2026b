...
bo_path = "/Users/tomas/Downloads/dm2026b_src_workflows_z910_WorkFlow_01_junior con BO.ipynb"
ensemble_path = "/Users/tomas/Downloads/dm2026b_src_workflows_z910_WorkFlow_01_junior con BO y ensemble.ipynb"
ensemble_dev = "/Users/tomas/Dev/dm2026b/src/workflows/dm2026b_src_workflows_z910_WorkFlow_01_junior con BO y ensemble.ipynb"

with open(base_clean_path, "r", encoding="utf-8") as f:
    nb = json.load(f)

# 1. Update Cell 11 for BO
nb_bo = copy.deepcopy(nb)
nb_bo["cells"][11]["source"] = [
    "PARAM <- list()\n",
    "PARAM$semilla_primigenia <- 102191\n",
    "\n",
    "PARAM$experimento <- \"semilla 102191 - metodo rank_cero_fijo- Jr Septiembre con BO\"\n",
...
nb_bo["cells"][67]["source"] = bo_code

with open(bo_path, "w", encoding="utf-8") as f:
    json.dump(nb_bo, f, indent=2, ensure_ascii=False)

# 2. Build Ensemble Notebook
nb_ensemble = copy.deepcopy(nb_bo)
nb_ensemble["cells"][11]["source"] = [
    "PARAM <- list()\n",
    "PARAM$semilla_primigenia <- 102191\n",
    "\n",
    "PARAM$experimento <- \"semilla 102191 - metodo rank_cero_fijo- Jr Septiembre con BO y ensemble\"\n",
    "PARAM$dataset <- \"analistajr_competencia_2026.csv.gz\"\n"
]
...
new_ensemble_cells = nb_ensemble["cells"][:78] + [c_78, c_79, c_80, c_81] + nb_ensemble["cells"][88:]
nb_ensemble["cells"] = new_ensemble_cells

with open(ensemble_path, "w", encoding="utf-8") as f:
    json.dump(nb_ensemble, f, indent=2, ensure_ascii=False)

with open(ensemble_dev, "w", encoding="utf-8") as f:
    json.dump(nb_ensemble, f, indent=2, ensure_ascii=False)

print("COMPLETED PERFECT CREATION!")
