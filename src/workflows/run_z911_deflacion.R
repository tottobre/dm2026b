# Auto-generated R script from z911_WorkFlow_01_junior_julio (deflacion)- semilla 900179.ipynb
...
# --- CELL 11 ---
PARAM <- list()
PARAM$semilla_primigenia <- 900179

PARAM$experimento <- "WFsemilla_900179-deflacion"
...
dir.create(file.path(base_exp, experimento_folder), recursive=TRUE, showWarnings=FALSE)
setwd(file.path(base_exp, experimento_folder))
cat("Working directory set to:", getwd(), "\n")


# --- CELL 16 ---
cat(format(Sys.time(), "[%H:%M:%S]"), "Reading dataset...\n")
# lectura del dataset local
dataset_path <- file.path("/Users/tomas/Dev/dm2026b/datasets", PARAM$dataset)
cat("Reading dataset from:", dataset_path, "\n")
dataset <- fread(dataset_path)


# --- CELL 19 ---
# mice commented out
# require("mice") commented out
...
# --- CELL 38 ---
cat(format(Sys.time(), "[%H:%M:%S]"), "Applying Data Drifting (deflacion)...\n")
...
# --- CELL 48 ---
cat(format(Sys.time(), "[%H:%M:%S]"), "Generating Feature Engineering Historico (lags)...\n")
...
PARAM$trainingstrategy$training <- c(202107)
PARAM$trainingstrategy$training_pct <- 0.75
PARAM$trainingstrategy$positivos <- c("BAJA+2")
...
# Split 202107 for train (75%) and validate (25%)
dataset[, fold_train := (foto_mes == 202107) & (azar < 0.75)]
dataset[, fold_validate := (foto_mes == 202107) & (azar >= 0.75)]
...
cat(format(Sys.time(), "[%H:%M:%S]"), "Starting LightGBM Grid Search...\n")
...
cat(format(Sys.time(), "[%H:%M:%S]"), "Predicting on future dataset...\n")
prediccion <- predict(
  final_model,
