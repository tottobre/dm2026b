...
out_dir = '/Users/tomas/Dev/dm2026b/src/workflows/septiembre_notebooks'
os.makedirs(out_dir, exist_ok=True)

seeds = [401987, 456791, 607219, 701819, 811147]
methods = ['rank_cero_fijo', 'deflacion', 'ninguno', 'rank_simple']

def make_grid_ipynb(seed, method):
    code_cells = [
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": [
                f"# Workflow Junior Septiembre (202109)\n",
                f"### Semilla: `{seed}` | Método Data Drift: `{method}` | Estrategia: `Grid Search`\n",
                "Validation fold: `202107` | Target prediction: `202109` (Kaggle Submission)"
            ]
        },
        {
            "cell_type": "code",
            ...
            "source": [
                ...
                "\n",
                "PARAM <- list()\n",
                f"PARAM$semilla_primigenia <- {seed}\n",
                f"PARAM$experimento <- \"WF_sep_semilla_{seed}-{method}\"\n",
        ...
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## Training Strategy (Validation on 202107)"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
        ...
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## Grid Search Optimization"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
        ...
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## Final Model Training & Future Prediction (202109 Kaggle)"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
...
def make_bo_ipynb(seed, method):
    code_cells = [
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": [
                f"# Workflow Junior Septiembre (202109) con BO\n",
                f"### Semilla: `{seed}` | Método Data Drift: `{method}` | Estrategia: `Optimización Bayesiana (mlrMBO)`\n",
                "Validation fold: `202107` | Target prediction: `202109` (Kaggle Submission)"
            ]
        },
        {
            ...
            "source": [
                ...
                "PARAM <- list()\n",
                f"PARAM$semilla_primigenia <- {seed}\n",
                f"PARAM$experimento <- \"WF_sep_semilla_{seed}-{method}_BO\"\n",
                "PARAM$dataset <- \"analistajr_competencia_2026.csv.gz\"\n",
                "\n",
                "dir.create(paste0(\"/Users/tomas/Dev/dm2026b/exp/\", PARAM$experimento), showWarnings = FALSE, recursive = TRUE)\n",
                "setwd(paste0(\"/Users/tomas/Dev/dm2026b/exp/\", PARAM$experimento))\n",
        ...
        },
        {
            "cell_type": "markdown",
            "metadata": {},
            "source": ["## Bayesian Optimization (mlrMBO)"]
        },
        {
            "cell_type": "code",
            "execution_count": None,
...
for seed in seeds:
    for method in methods:
        # Grid notebook
        nb_grid = make_grid_ipynb(seed, method)
        path_grid = os.path.join(out_dir, f"z910_WF_septiembre_grid_semilla_{seed}_{method}.ipynb")
        with open(path_grid, "w") as f:
            json.dump(nb_grid, f, indent=2)
        total += 1
        
        # BO notebook
        nb_bo = make_bo_ipynb(seed, method)
        path_bo = os.path.join(out_dir, f"z910_WF_septiembre_bo_semilla_{seed}_{method}.ipynb")
        with open(path_bo, "w") as f:
            json.dump(nb_bo, f, indent=2)
        total += 1

print(f"Created {total} .ipynb notebooks in {out_dir}")
